...
root = tk.Tk()
root.title("ComboTool GUI Platinum Pro Premium Plus by noarch")
...
def extract_by_search():
    ...
    if search_term:
        matched_lines = [line for line in input_text.get(1.0, tk.END).splitlines() if search_term in line]
...
def show_domain_statistics():
    ...
    lines = input_text.get(1.0, tk.END).splitlines()
    domains = [re.search(r"@(.+?)\.", line) for line in lines]
    domain_list = [match.group(1) for match in domains if match]
